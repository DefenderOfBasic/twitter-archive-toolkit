export class Util {
    ...
    async fetchArchive(archiveUrl) {
      const storedData = await this.db.tweets.get(1)
      let archiveJson

      if (storedData && storedData.url === archiveUrl) {
        console.log("Getting stored tweet data from IndexedDB")
        archiveJson = storedData.data
      } else {
